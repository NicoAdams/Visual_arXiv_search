...
function radius(d) {
  var connections = graphData.links.filter(l =>   l.target  == d );
  return(Math.max(0,(connections.length-1)*5))
}
drag = simulation => {
...
graph = Object.assign(svg.node(), {
    update({nodes, links}) {
      ...
      links = links.filter(l =>   (radius(l.target)>0 && radius(l.source)>0))
      nodes = nodes.filter(d =>   radius(d.id)>0)
      ...
      nodeGroup = nodeGroup
        ...
        .join(function(enter){
          ...
          nodeG.append("circle")
          .attr("r",d=>  radius(d.id))
          .attr("fill", d=> color(d.firstAuthor))
